...
__author__ = """Thomas Aynaud (thomas.aynaud@lip6.fr); Modified by Maksim Tsvetovat (maksim@tsvetovat.orgPAR)"""
#    Copyright (C) 2009 by
...
def plot_community(G,filename='communities.pdf'):
    ...
    #drawing
    plot.figure(num=None, figsize=(64, 48), facecolor='w', edgecolor='k')
    ...
    for com in set(partition.values()) :
        ...
        net.draw_networkx_nodes(G, pos, list_nodes, node_size =70,
                                    node_color=colors[count])


    net.draw_networkx_edges(G,pos, alpha=0.5)
    net.draw_networkx_labels(G,pos,font_size=10)
    plot.savefig(filename)
    plot.show()

def partition_subgraphs(G):
    subs=[]
    partition = best_partition(G)
    for com in set(partition.values()) :
        list_nodes = [nodes for nodes in partition.keys() if partition[nodes] == com]
        subs.append(G.subgraph(list_nodes))
    return subs

def plot_partitions(G,filename):    
    subgraphs=partition_subgraphs(G)
    colors=['r','g','b','c','m','y','k']
    count=0
    for i,sub in enumerate(subgraphs):
        if len(sub)<10: continue
        d=net.degree(sub)
        ns=[d[n]*10 for n in sub.nodes()]
        count+=1
        if count >= len(colors): count=0
        plot.figure(num=None, figsize=(16, 12), dpi=180, facecolor='w', edgecolor='k')
        net.draw_networkx(sub,node_size=ns,node_color=colors[count])
        plot.savefig(filename+str(i)+'.pdf')
        plot.clf()
        
        w=csv.writer(open(filename+'_list_'+str(i)+'.csv','wb'))
        w.writerows(sorted_dict(d))
